...
#define EQUAL(w1, w2) (std::equal((w1).begin(), (w1).end(), (w2).begin()))
    ...
    void localization(void)
    {
        ...
        while (P.size() > 1) {
            ...
            if (DEBUG)
                ROS_INFO("localization: curr: %s", WallsToString(curr).c_str());
            ...
            for (std::vector<cell>::iterator ic = P.begin(); ic != P.end(); ++ic) {
                ...
                if (DEBUG)
                    ROS_INFO("localization: curr_w: (%d,%d) %s", (*ic).coordinates.first, (*ic).coordinates.second,
                ...
                for (int i = 0; i < 4; i++) {
                    if (EQUAL(curr, curr_w)) {
                        ...
                        if (DEBUG)
                            ROS_INFO("localization: pos: [%d,%d,%d]", pos.back()[0], pos.back()[1], pos.back()[2]);
                    ...
                    if (DEBUG)
                        ROS_INFO("localization: -->r curr_w: %s|%c", WallsToString(curr_w).c_str(),
        ...
        if (DEBUG)
            ROS_INFO("localization: pos.size()=%d, curr.size()=%d, wp=%s, o=%d", (int)pos.size(), (int)curr.size(),
                     wp_lookup[wp].c_str(), orientation);

        if (DEBUG)
            ROS_INFO("localization: P.size()=%d", (int)P.size());
    }
